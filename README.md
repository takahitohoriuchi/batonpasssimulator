...
| ID | 各時点 | Pの状態 | Rの状態 | その時点に遷移してくる条件 | 備考 |
|---|---|---|---|---|---|
| 1 | 初期状態 | `pStage=0` 走っている | `rStage=0` 待ち | レース開始直後、またはリセット直後 | P がバトン保持者。R はまだ出走していない。 |
| 2 | R出走後 | `pStage=0` 走っている | `rStage=1` 出走済 | R操作キー入力 | Mac は `Control`、Windows は `Tab`。`R.go()` が呼ばれる。 |
| 3 | P声かけ後 | `pStage=1` 声かけ済 | `rStage=1` 出走済 | `Enter` 入力 | `called=true` になり、「はい！」表示が一定時間出る。 |
| 4 | Rもらうポーズ後 | `pStage=1` 声かけ済 | `rStage=2` もらうポーズ | call後に R の位相が `3π/2` を跨ぐ | `R.enterReceiveReady()` により腕上げ状態に入る。 |
| 5 | P差し出し後 | `pStage=2` 差し出し済 | `rStage=2` もらうポーズ | `canOfferNow=true` | P が差し出し姿勢に入る。P/R ともに受け渡し中フラグが立つ。 |
| 6 | R把持後 | `pStage=2` 差し出し済 | `rStage=3` 握り済 | R操作キー入力、かつ `canGrasp()` が真 | この時点ではまだバトンの holder は P。 |
| 7 | P手放し後 | `pStage=3` 手放し済 | `rStage=3` 握り済 | `Enter` 入力 | バトン holder が R に移る。成功。 |
| 8 | 失敗: 接触 | 途中段階のまま停止 | 途中段階のまま停止 | `P.dist >= R.dist` | P が R に追いついて衝突した失敗。 |
| 9 | 失敗: ゾーン超過 | `pStage<3` のまま | R がゾーン終端を越える | `R.dist > zone.end` かつ release 前 | 手放し完了前にゾーン終端を越えた失敗。 |

### `canOffer` の条件

P が「差し出してよい」と判定される条件は、現在のコードでは次の3つです。

1. R がすでに「もらうポーズ」に入っていること
2. P の位相 `phi` が `π <= phi < 1.5π` の範囲にあること
3. P と R の距離が `dif <= R.l - sin(phi) * P.l` を満たすこと
